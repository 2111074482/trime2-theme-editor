...
public final class ThemeEditorWorkspace extends LinearLayout {
    ...
    public ThemeEditorWorkspace(Context context) {
        ...
        toolbar.addView(addButton); toolbar.addView(duplicateButton); toolbar.addView(deleteButton); toolbar.addView(undoButton); toolbar.addView(redoButton); toolbar.addView(saveButton); addView(toolbar, new LayoutParams(-1, -2));
        ...
        canvas.setListener(new ThemeKeyboardCanvas.Listener() { public void onKeySelected(ThemeEditorModel.Key key) { properties.bind(key); setStatus(key == null ? "No selection" : "Selected " + key.label); if (callbacks != null) callbacks.onSelectionChanged(key); } public void onKeyMoveStarted() { changeStarted(); } public void onKeyMoved() { setStatus("Move key, release to finish"); if (callbacks != null) callbacks.onModelChanged(model.copy()); } });
    ...
    private void addKey() {
        changeStarted();
        int index = model.keys.size();
        ThemeEditorModel.Key key = new ThemeEditorModel.Key("new_key_" + index, "new", 10 + (index % 8) * 11, 8 + (index / 8) * 18, 9.5f, 16);
        model.keys.add(key); canvas.setModel(model); canvas.setSelectedKey(key); properties.bind(key); setStatus("Added key");
        if (callbacks != null) callbacks.onModelChanged(model.copy());
    }
    ...
    private void duplicateSelected() {
        ...
        changeStarted();
        ThemeEditorModel.Key copy = selected.copy(); copy.id = selected.id + "_copy_" + model.keys.size(); copy.x = Math.min(100 - copy.width, copy.x + 2);
        model.keys.add(copy); canvas.setModel(model); canvas.setSelectedKey(copy); properties.bind(copy); setStatus("Copied key");
        if (callbacks != null) callbacks.onModelChanged(model.copy());
    }
    ...
    private void deleteSelected() {
        ThemeEditorModel.Key selected = canvas.getSelectedKey();
        if (selected == null) { setStatus("Select a key first"); return; }
        changeStarted(); model.keys.remove(selected); canvas.setSelectedKey(null); canvas.setModel(model); properties.bind(null); setStatus("Deleted key");
        if (callbacks != null) callbacks.onModelChanged(model.copy());
    }
