...
public class LuaLayout {
    ...
    private LuaValue layoutParams = CoerceJavaToLua.coerce(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT));
    ...
    public LuaValue load(LuaValue layout, LuaTable env, LuaValue params) {
        ...
        if (cls.isnil())
            throw new LuaError("loadlayout error: First value Must be a Class, checked import package.\\n\\tat " + layout.checktable().dump());
        boolean isAdapterView = cls.isuserdata() && AdapterView.class.isAssignableFrom(cls.touserdata(Class.class));
        JavaInstance view = (JavaInstance) cls.call(mContext);
        params = params.call(layoutParams);
        try {
                    ...
                    if (key.isint()) {
                        if (key.toint() > 1) {
                            ...
                            if (isAdapterView) {
                                ...
                                view.getJavaMethod("addView").call(v);
                            }
                        }
                        ...
                        switch (k) {
                            ...
                            case "textSize":
                                if (val.isnumber())
                                    view.getJavaMethod("setTextSize").call(val.tonumber());
                                else
                                    view.getJavaMethod("setTextSize").jcall(0, toint(val.tojstring()));
                                continue;
                            case "scaleType":
                                view.getJavaMethod("setScaleType").jcall(ImageView.ScaleType.values()[scaleType.get(val.tojstring())]);
                                continue;
                            case "ellipsize":
                                view.getJavaMethod("setEllipsize").jcall(TextUtils.TruncateAt.valueOf(val.tojstring().toUpperCase()));
                                continue;
                            case "hint":
                            ...
                            case "items":
                                ...
                                if (!adapter.isnil()) {
                                    ...
                                    view.getJavaMethod("setAdapter").jcall(new ArrayListAdapter<>(mContext.touserdata(Context.class), android.R.layout.simple_list_item_1, (String[]) CoerceLuaToJava.arrayCoerce(val, String.class)));
                                }
                                continue;
                            case "pages":
                                ...
                                view.getJavaMethod("setAdapter").jcall(new ArrayPageAdapter(vs));
                                continue;
                            case "src":
                                try {
            ...
            try {
                ...
                if (sp)
                    view.getJavaMethod("setPadding").invoke(LuaValue.varargsOf(pds));
            } catch (Exception e) {
