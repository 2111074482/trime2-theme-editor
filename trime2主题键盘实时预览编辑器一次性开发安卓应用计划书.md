...
每个阶段必须独立满足“可编译、可启动、可回退、对应自动化测试通过”后才能进入下一阶段。不得等到阶段六才首次恢复测试依赖、运行测试或构建 APK。
...
- M7 Release:字段报告零缺失、自动测试通过、人工清单有结果、许可证和签名材料齐全。
